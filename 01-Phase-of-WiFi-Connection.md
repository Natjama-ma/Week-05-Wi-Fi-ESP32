...
~~~
เพราะสถานะนี้บอกแค่ว่าเชื่อมต่อกับ Wi-Fi แล้วเท่านั้น แต่ยังไม่ได้รับหมายเลข IP Address จาก Router
ต้องรอให้เกิด Event IP_EVENT_STA_GOT_IP ก่อน จึงจะสามารถสื่อสารผ่านอินเทอร์เน็ตไปยังเซิร์ฟเวอร์ภายนอกได้
~~~

2. หากพิมพ์รหัสผ่าน Wi-Fi ผิด กระบวนการเชื่อมต่อจะหลุดและแจ้ง Error ในเฟสใด?
~~~
หลุดในเฟส Authentication / Handshake (การยืนยันตัวตน)
~~~

3. ระบบ Event Loop มีข้อดีอย่างไรเมื่อเทียบกับการเขียนโค้ดแบบวนลูปเช็กสถานะ (Polling loop)?
~~~
1. ประหยัดพลังงานและทรัพยากร CPU
ไม่ต้องรันลูปวนเช็กสถานะตลอดเวลา ทำให้ CPU ไปทำงานอื่นหรือเข้าสู่โหมดประหยัดพลังงานได้
2. ตอบสนองไว ไม่บล็อกการทำงาน (Non-blocking)
โปรแกรมจะทำงานทันทีที่มี Event เกิดขึ้น (Reactive) โดยไม่ต้องเขียน delay() รอเช็กค่า
~~~
