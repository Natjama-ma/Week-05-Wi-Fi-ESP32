...
~~~
เพราะทั้งสองฝั่ง (Router และ ESP32) รู้รหัสผ่านล่วงหน้าอยู่แล้ว จึงใช้รหัสผ่านนำไปคำนวณสร้างเป็น PMK (Pairwise Master Key)
~~~
2. เหตุใดการพิมพ์รหัสผ่าน Wi-Fi ผิดใน ESP32 จึงมักหลุดที่ขั้นตอน **2/4 EAPOL** หรือ **3/4 EAPOL** ไม่ใช่ตั้งแต่เฟส 1 หรือ 2?
~~~
เพราะ เฟส 1 (Auth) และ เฟส 2 (Association) เป็นแค่ขั้นตอนการเชื่อมต่อทางกายภาพและลงทะเบียนอุปกรณ์ โดยยังไม่มีการตรวจสอบรหัสผ่าน
~~~
3. ความแตกต่างสำคัญระหว่าง Event **`WIFI_EVENT_STA_CONNECTED`** กับ **`IP_EVENT_STA_GOT_IP`** คืออะไร?
~~~
WIFI_EVENT_STA_CONNECTED (L2 - Data Link)
คือ การเชื่อมต่อทาง Wi-Fi และถอดรหัสผ่าน สำเร็จในระดับ Layer 2 เท่านั้น (อุปกรณ์จับคู่กับ Router ได้แล้ว)
ยังส่งข้อมูลทางอินเทอร์เน็ตไม่ได้

IP_EVENT_STA_GOT_IP (L3 - Network)
คือ การที่ ESP32 ได้รับหมายเลข IP Address จาก DHCP Server ของ Router เรียบร้อยแล้ว
พร้อมเริ่มส่ง-รับข้อมูล ไปยังเว็บเซิร์ฟเวอร์หรืออินเทอร์เน็ตได้ทันที
~~~
